...
const CHANNELS = [
  ...
  { id: 'social',  label: 'Social Ads',    icon: Monitor },
  { id: 'landing', label: 'Landing Page',  icon: FileText },
  { id: 'sms',     label: 'SMS',           icon: Smartphone },
]

// ── Campaign history (localStorage) ─────────────────────────
type CampaignHistoryItem = { name: string; date: string; channels: number }

function useCampaignHistory(): CampaignHistoryItem[] {
  const [history, setHistory] = useState<CampaignHistoryItem[]>([])
  useEffect(() => {
    try {
      const stored = JSON.parse(localStorage.getItem('cf_campaign_history') || '[]')
...
export default function HomePage() {
  ...
  const [selectedChannels, setSelectedChannels] = useState<string[]>(['email', 'social'])
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/generate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name, category, audience, goal, tone, channels: selectedChannels }),
      })
  ...
  return (
    <>
              ...
              <div className="card-forge" style={{ padding: '24px 24px 20px' }}>
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  ...
                  <div>
                    ...
                    <p style={{ color: 'var(--ink-4)', fontSize: 11, marginTop: 6 }}>
                      {selectedChannels.length} channel{selectedChannels.length !== 1 ? 's' : ''} selected — AI will generate assets for each
                    </p>
                  </div>
      ...
      {campaignHistory.length > 0 && (
        <section style={{ position: 'relative', zIndex: 10, borderTop: '1px solid var(--border-s)', padding: '28px 24px' }}>
          <div style={{ maxWidth: 1100, margin: '0 auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
              <History size={15} color="var(--forge)" />
              <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--ink-2)', letterSpacing: '0.04em', textTransform: 'uppercase' }}>Recent Campaigns</span>
            </div>
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
              {campaignHistory.map((c, i) => (
                <div key={i} style={{ padding: '10px 16px', background: 'var(--bg-raised)', border: '1px solid var(--border-d)', borderRadius: 10, display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--forge)', flexShrink: 0 }} />
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--ink-1)' }}>{c.name}</div>
                    <div style={{ fontSize: 11, color: 'var(--ink-4)', marginTop: 2 }}>{c.date} · {c.channels} channel{c.channels !== 1 ? 's' : ''}</div>
                  </div>
                </div>
              ))}
            </div>
